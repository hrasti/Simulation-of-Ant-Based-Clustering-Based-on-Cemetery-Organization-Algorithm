...
%{
First, generate a sample data and after setting the parameters press the "RUN" botton.
Note:
    ...
    for antNum = 1:numberOfAnts
        ...
        if (nextLocation(1) < latticeSize )&&(nextLocation(2) < latticeSize)&&...
            ...
            while condition
                ...
                if randNumber>directionProbability %% Change direction
        ...
        %% Pick up and Drop Procedure
        if isempty(ants(antNum).patternLoad) % if ant is empty and ready to pickup
            if (~isempty(lattice(ants(antNum).location(1) , ants(antNum).location(2)).pattern))&&(t==0) % Pickup a single pattern with no neighbors around
            ...
            elseif (~isempty(lattice(ants(antNum).location(1) , ants(antNum).location(2)).pattern))&&(t>0)
                ...
                d = 0;
                %% Calculate similarity between the pattern in the candidate cell with its neighbors
                for nD = 1:t
                ...
                randNumber = rand;
                if randNumber < pickupP  % pick up if you should
                    ants(antNum).patternLoad =lattice(ants(antNum).location(1) , ants(antNum).location(2)).pattern;
                    lattice(ants(antNum).location(1) , ants(antNum).location(2)).pattern = [];
                end
            end
        else % if ant carry a pattern and ready for drop
            ...
            elseif (isempty(lattice(ants(antNum).location(1) , ants(antNum).location(2)).pattern))&&(t>0)
                ...
                randNumber = rand;
                if randNumber < dropP % drop if you should
                    lattice(ants(antNum).location(1) , ants(antNum).location(2)).pattern = ants(antNum).patternLoad;
                    ants(antNum).patternLoad =[];
                end
...
% msgbox('Click on the lattice and press any key to continue...')
% pause on
% pause
...
function cantinueBtn_Callback(hObject, eventdata, handles)
